...
public class Tests
{
  ...
  @Test
  public void should_detect_full_house_one_twos_one_threes ()
  {
    List<Card> hand = new ArrayList<Card>();
    List<Card> communityCards = new ArrayList<Card>();
  ...
  @SuppressWarnings("unchecked")
  @Test
  public void should_detect_full_house_zero_twos_two_threes ()
  {
    List<Card> hand = new ArrayList<Card>();
    List<Card> communityCards = new ArrayList<Card>();
  ...
  public void should_detect_royal_flush ()
  {
    ...
    ActualPokerHand pokerHand = sut.checkForHand(hand, communityCards);
    PokerHandEnum pokerHandEnum = pokerHand.getPokerHandEnum();
    assertThat(pokerHandEnum, is(PokerHandEnum.ROYAL_FLUSH));
    assertThat(pokerHand.getWinningCards().get(0).getValue(), is(Value.TEN));
    assertThat(pokerHand.getWinningCards().get(1).getValue(), is(Value.JACK));
    assertThat(pokerHand.getWinningCards().get(2).getValue(), is(Value.QUEEN));
    assertThat(pokerHand.getWinningCards().get(3).getValue(), is(Value.KING));
  ...
  @SuppressWarnings("unchecked")
  @Test
  public void should_result_in_win_with_two_pair_only ()
  {
    List<Card> communityCards = new ArrayList<Card>();
    communityCards.add(new Card(Suit.CLUBS, Value.TEN));
    ...
    setupPlayer(players, "player2", new Card(Suit.SPADES, Value.QUEEN), new Card(Suit.DIAMONDS, Value.QUEEN));

    sut.setPlayers(players);

    String determineWinner = sut.determineWinner();

    assertThat(determineWinner, anyOf(is("player2[QUEEN of SPADES, QUEEN of DIAMONDS] wins with Two pair - "), is("player2[QUEEN of SPADES, QUEEN of DIAMONDS] wins with Two pair - ")));
  }
